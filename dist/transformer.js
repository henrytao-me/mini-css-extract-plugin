...
module.exports = (loaderContext, options, namespaces) => {
  ...
  // Write *.[style].js file
  fs.outputFileSync(`${resolve(outDir, relativeResourcePath)}.js`, `module.exports = ${JSON.stringify(namespaces)}`);

  // Append scss file to scssFileDir
  if (scssFileDir) {
    fs.outputFileSync(scssFileDir, `${fs.readFileSync(scssFileDir, 'utf8')}@import '.${resolve(scssDir, relativeResourcePath).replace(resolve(scssFileDir, '../'), '')}';\r\n`);
  }
};